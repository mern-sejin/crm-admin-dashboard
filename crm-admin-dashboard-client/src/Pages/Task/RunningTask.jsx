...
export default function RunningTask () {
    ...
    return (
        <Box component='section' className='page-layout'>
            <Helmet>
                <title>CRM Admin Dashboard | Running Tasks</title>
            </Helmet>
            <PageHeader icon={<LuChartColumnIncreasing/>} title='Running Tasks' subTitle='Running Tasks details'/>
            <Box component='div' p={4} px={{ xs: 2, sm: 4 }} pb={2}>
                ...
                <FullScreen handle={fullscreen}>
                    <Draggable nodeRef={element} disabled={!enableDrag} position={{ x: 0, y: 0 }}>
                        ...
                        boxShadow='0 3px 6px #00000029,0 3px 6px #0000003b'>
                            <PanelHeader title='Add Admin' enableDrag={enableDrag} setEnableDrag={setEnableDrag} 
...
const Timer = () => {
    const time = new Date();
    const second = time.setSeconds(time.getSeconds() + 19800);
    const { seconds, minutes, hours, restart } = useTimer({ 
        expiryTimestamp: second, 
        onExpire: () => restart(second), 
    });
    return (
        <Box component='div' textAlign='center'>
            <Typography component='span' fontSize={20} fontWeight={500}>
                {hours} Hours, {minutes} Minutes, {seconds} Seconds
            </Typography>
        </Box>
    );
};
const RunningTasksDuration = () => {
    const fullscreen = useFullScreenHandle();
    const [enableDrag, setEnableDrag] = useState();
    const element = useRef(null);
    return (
        <FullScreen handle={fullscreen}>
            <Draggable nodeRef={element} disabled={!enableDrag} position={{ x: 0, y: 0 }}>
                <Box component='div' height={fullscreen.active && '100vh'} bgcolor='white' ref={element}
                boxShadow='0 3px 6px #00000029,0 3px 6px #0000003b'>
                    <PanelHeader title='Tasks Duration' enableDrag={enableDrag} setEnableDrag={setEnableDrag} 
                    fullscreen={fullscreen}/>
...
const AddNewTask = () => {
    ...
    return (
        <Box component='div'>
            <Button variant='text' sx={{ bgcolor: 'var(--main)', color: 'white', px: 2, gap: 1 }} onClick={handleOpen}>
                <TiUserAdd size={20}/>
                Add Users
            </Button>
                ...
                <Box sx={style}> 
                    ...
                    borderBottom='1px solid lightgray' p={2}>
                        <Box component='div' display='flex' alignItems='center' gap={1} color='#374767'>
                            <GoTasklist size={30}/>
                            <Typography component='span' fontSize='large' fontWeight={600}>Add New Task</Typography>
                        </Box>
                        <Box component='div'>
